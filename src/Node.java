...
class Node implements Comparable<Object> {
	...
	// Node constructor for one letter symbols
	public Node(char content, double value) {
		this.content = content;
		this.value = value;
	}

	// Node constructor for two letter symbols
	...
	public Node(Node left, Node right, boolean two) {
		if (!two) {
			...
			// Assumes that the left three is always the one that is lowest
			this.tuple = (left.tuple.compareTo(right.tuple) < 0) ? left.tuple
					: right.tuple;
			this.value = left.value + right.value;
			this.left = left;
			this.right = right;
	...
	// Recursively organizes the tree into appropriate managers for encoding and
	// decoding
